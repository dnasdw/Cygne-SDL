  ...
  if((fp = unzOpen(RomPath)))
  {
    ...
    if(unzGoToFirstFile(fp) == UNZ_OK)
    {
      do
      {
        ...
        {
          if(strcasecmp(&szFileName[strlen(szFileName) - 3], ".ws") == 0
            || strcasecmp(&szFileName[strlen(szFileName) - 4], ".wsc") == 0)
           {
...
void wsROMLoad(const char *filenameTo)
{
	...
	strcpy(buf, filenameTo);
	if(buf[0])
	{
		 strcpy(romname,"Cygne - ");
		 strcat(romname,buf);
		 if(!strcasecmp(&buf[strlen(buf) - 4], ".zip"))
...
void wsLoadState(const char *filename)
{
	char			buffer[1024];
	char			text1[1024];
	char			cx[1024];
...
void wsSaveState(const char *filename)
{
	char			cBuffer[1024];
	FILE*			fState;
	unsigned		uReg;
